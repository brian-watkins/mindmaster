      ...
      |> observeThat
        [ it "sends the guess to the evaluator" (
            Witness.observe "guess-to-evaluate" (Json.list Json.string)
              |> expect (isList
                [ equals [ "red", "orange", "yellow", "green", "blue" ]
                ]
              )
          )
...
testSubjectWithEvaluator positions guessesRemaining evaluator =
  Subject.initWithModel (testModel positions)
    |> Witness.forUpdate (testUpdate evaluator)
...
testView guessesRemaining =
  UI.View.with <| InProgress guessesRemaining
